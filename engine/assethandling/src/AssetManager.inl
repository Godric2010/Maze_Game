...
namespace Engine::AssetHandling {
    ...
    std::vector<AssetHandler::HandleT<T> > AssetHandler::LoadAssets(
        ...
        auto assets = AssetTraits<T>::LoadMany(context, asset_files);
        std::vector<HandleT<T> > handles;
        handles.reserve(assets.size());
        for (auto& asset: assets) {
            typename AssetTraits<T>::Handle handle{cache.next_id++};
            cache.id_by_name.emplace(asset.name, handle);
            cache.by_id.emplace(handle,
                                AssetRecord<T>{
                                    .asset = std::make_shared<T>(std::move(asset)),
                                    .revision = 1,
