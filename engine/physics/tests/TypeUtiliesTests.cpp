...
TEST_CASE("Sphere Utilities - Resulting bounding box is as expected") {
    SECTION(
        ...
        constexpr auto s = yarep::physics::collision::Sphere{
            .center = glm::vec3{4, 2, 1},
            .radius = 5
        };

        const auto [min, max] = yarep::physics::collision::util::FromSphere(s);
    ...
    SECTION("Sphere with negative radius - No Bounding box gets build") {
        ...
        REQUIRE_THROWS(yarep::physics::collision::util::FromSphere(s));
    }
...
TEST_CASE("Orientated Bounding Box") {
    ...
    SECTION("Create OBB with no rotation - All values are as expected") {
        ...
        auto obb = yarep::physics::collision::util::BuildWorldObb(position, glm::vec3(0), width, height, depth);
