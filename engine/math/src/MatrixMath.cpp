...
namespace yarep::math {
    Mat3 transpose(const Mat3& matrix) {
        const glm::mat3 glm_mat = detail::to_glm_mat3(matrix);
        const auto transposed_glm_mat = glm::transpose(glm_mat);
        return detail::from_glm_mat3(transposed_glm_mat);
    }

    Mat4 transpose(const Mat4& matrix) {
        const glm::mat4 glm_mat = detail::to_glm_mat4(matrix);
        const auto transposed_glm_mat = glm::transpose(glm_mat);
        return detail::from_glm_mat4(transposed_glm_mat);
    }
    ...
    Mat3 inverse(const Mat3& matrix) {
        ...
        return detail::from_glm_mat3(inverse_glm_mat);
    }
    ...
    Mat4 inverse(const Mat4& matrix) {
        ...
        const auto inverse_glm_mat = glm::inverse(glm_mat);
        return detail::from_glm_mat4(inverse_glm_mat);
    }
    ...
    Mat4 translation_matrix(const Vec3& translation) {
        const auto translation_glm_vec = detail::to_glm_vec3(translation);
        const auto translate_glm_mat = glm::translate(glm::mat4(1.0f), translation_glm_vec);
        return detail::from_glm_mat4(translate_glm_mat);
    }

    Mat4 scale_matrix(const Vec3& scale) {
        const auto scale_glm_vec = detail::to_glm_vec3(scale);
        const auto scale_glm_mat = glm::scale(glm::mat4(1.0f), scale_glm_vec);
        return detail::from_glm_mat4(scale_glm_mat);
    }

    Mat4 rotation_matrix(const Quaternion& rotation) {
        const auto rotation_glm_quat = detail::to_glm_quat(rotation);
        const auto mat_glm = glm::mat4_cast(rotation_glm_quat);
        return detail::from_glm_mat4(mat_glm);
    }
    ...
    Mat4 look_at(const Vec3& eye, const Vec3& target, const Vec3& up) {
        ...
        const auto eye_glm_vec = detail::to_glm_vec3(eye);
        const auto target_glm_vec = detail::to_glm_vec3(target);
        const auto up_glm_vec = detail::to_glm_vec3(up);
        const auto look_at_glm = glm::lookAtRH(eye_glm_vec, target_glm_vec, up_glm_vec);
        return detail::from_glm_mat4(look_at_glm);
    }
    ...
    Mat4 perspective(Angle vertical_fov, float aspect_ratio, float near_plane, float far_plane) {
        ...
        return detail::from_glm_mat4(perspective_glm);
    }
    ...
    Mat4 orthographic(float left, float right, float bottom, float top, float near_plane, float far_plane) {
        ...
        return detail::from_glm_mat4(ortho_glm);
    }

    Mat3 normal_matrix(const Mat4& model_matrix) {
        const auto model_matrix_glm = detail::to_glm_mat4(model_matrix);
        ...
        return detail::from_glm_mat3(normal_glm_mat);
    }
}
