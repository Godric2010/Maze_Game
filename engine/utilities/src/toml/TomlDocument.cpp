...
namespace yarep::utilities::toml_utils {
    ...
    math::Vec2 TomlTable::GetRequiredVec2(const std::string& field_name) const {
    ...
    std::optional<math::Vec2> TomlTable::GetOptionalVec2(const std::string& field_name) const {
    ...
    std::optional<math::Vec4> TomlTable::GetOptionalVec4(const std::string& field_name) const {
        const auto array = m_node[field_name].as_array();
        if (!array) {
            return std::nullopt;
        }
        ...
        return math::Vec4(x.value(), y.value(), z.value(), w.value());
    }
